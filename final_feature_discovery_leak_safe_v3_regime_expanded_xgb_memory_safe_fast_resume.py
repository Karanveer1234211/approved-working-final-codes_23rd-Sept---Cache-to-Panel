...
VERSION = "2026-09-25.master.v3.regime_expanded.xgb_memory_safe_fast_resume"
SEED = 1729
...
def add_existing_panel_feature_expansions(
    ...
    """Expand original numeric panel features in bounded blocks.

    PERFORMANCE/MEMORY ONLY:
    The feature set, formulas, names, ordering, and candidate metadata are
    unchanged. Generated columns are attached in small blocks rather than
    one giant block or one column at a time. This avoids the ~9 GiB temporary
    consolidation allocation while avoiding pandas' severe fragmentation cost.
    """
    x = work_df
    base_id = x["_row_id"].to_numpy(copy=False)
    ...
    def flush_batch() -> None:
        ...
        if not batch:
            return
        block = pd.DataFrame(batch, index=x.index)
        x = pd.concat([x, block], axis=1, copy=False)
        batch = {}
        del block
    ...
    for c in source_cols:
        ...
        for name, series, family, lb, hyp in specs:
            if name in existing_names:
                continue
            batch[name] = pd.to_numeric(series, errors="coerce").astype("float32")
        ...
        if len(batch) >= EXPANSION_BATCH_SOURCE_FEATURES * 4:
            flush_batch()
...
def main():
    ...
    manifest = {
        ...
        "performance_optimizations": {
            "expansion_batch_source_features": EXPANSION_BATCH_SOURCE_FEATURES,

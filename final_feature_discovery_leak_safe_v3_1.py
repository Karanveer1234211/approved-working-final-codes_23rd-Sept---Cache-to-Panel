...
def empirical_max_null(
    ...
    For each permutation, shuffle the target across rows WITHIN EACH DATE.
    This preserves each day's cross-sectional class count while destroying
    feature/target association. We then test all candidates against the same
    ...
    for pno in range(permutations):
        ...
        shuffled = df.copy()
        shuffled[target] = (
            shuffled.groupby("_date", sort=False)[target]
            .transform(lambda s: s.sample(frac=1.0, random_state=int(rng.integers(0, 2**31 - 1))).to_numpy())
        )
...
def add_causal_regimes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Regimes use information through t only.
    # Shift rolling thresholds by one observation so the regime describes the
    # prior state rather than being partially defined by the current bar.
    atr = None
    ...
    if atr is None:
        atr = x["close"].groupby(x["symbol"], sort=False).pct_change().abs()
    ...
    prior_atr = atr_roll.groupby(x["symbol"], sort=False).shift(1)

    mom = x["close"].groupby(x["symbol"], sort=False).pct_change(20)
    prior_mom = mom.groupby(x["symbol"], sort=False).shift(1)

    x["REG_vol_high"] = (atr > prior_atr).astype("int8")
    x["REG_trend_up"] = (prior_mom > 0).astype("int8")
    x["REG_trend_down"] = (prior_mom < 0).astype("int8")

    x["REG_trend_vol"] = (
        x["REG_trend_up"].astype(str)
        + "_VH" + x["REG_vol_high"].astype(str)
    )

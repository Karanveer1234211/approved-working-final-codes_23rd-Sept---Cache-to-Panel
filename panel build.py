...
# The unsuffixed label columns (label_tp_before_sl, label_first_touch,
# label_days_to_*, label_same_day_ambiguous, label_exit_ret) mirror
# PRIMARY_VARIANT. v31 also carried LABEL_TP = 0.05 / LABEL_SL = 0.03 and
# wrote them to panel_meta.json, which then described a 5%/3% target while the
# primary label was 1.5 x ATR / 1.0 x ATR. Removed; see label_contract().

_BASE_LABELS = (
    "label_touch", "label_fwd_ret_5d", "label_mfe_5d", "label_mae_5d",
    "label_first_touch", "label_tp_before_sl", "label_days_to_tp",
    "label_days_to_sl", "label_same_day_ambiguous", "label_exit_ret",
)
_VARIANT_LABELS = tuple(
    f"label_{f}_{suf}"
    for suf, _, _, _ in LABEL_VARIANTS
    for f in ("tp_before_sl", "days_to_tp", "same_day_ambiguous", "touch", "exit_ret")
)
...
def compute_labels(
    ...
    def _variant(tp, sl, use_atr):
        ...
        exit_r = np.full(n, np.nan)
        if not len(idx):
            return first, d_tp, d_sl, amb, touch, exit_r
        if use_atr:
        ...
        # The trade's REALISED gross return: +TP distance if the target came
        # first, -SL distance if the stop came first (same-day ties count as
        # the stop, matching the label), otherwise the close-to-close return
        # at the horizon. Stored so that every tool computes economics from
        # the exact barriers used here - D_atr14 is Wilder-smoothed, this is
        # a simple 14-day mean, and the two differ.
        with np.errstate(invalid="ignore", divide="ignore"):
            fwd = c_arr[idx + hz] / c_arr[idx] - 1.0
        er = np.where(res == 1.0, lvl_tp, np.where(res == -1.0, -lvl_sl, fwd))
        exit_r[idx] = np.where(ok, er, np.nan)
        return first, d_tp, d_sl, amb, touch, exit_r

    new = {}
    for suf, tp, sl, use_atr in LABEL_VARIANTS:
        f, dtp, dsl, amb, tch, xr = _variant(tp, sl, use_atr)
...
def symbol_frame(root: Path, symbol: str, since: Optional[pd.Timestamp]) -> pd.DataFrame:
    ...
    if feat.empty:
        return pd.DataFrame()
    # Forward-looking columns never reach the panel file. ret_5d_close_pct is
    # close[t+5]/close[t]-1; label_fwd_ret_5d already carries that outcome as
    # a LABEL. Stored under a non-label name it was one filter away from
    # becoming a feature - and the atlas did not use that filter.
    import research_common as RC
    feat = feat.drop(columns=[c for c in RC.FORWARD_COLUMNS if c in feat.columns])
...
def apply_liquidity_floor(
    ...
    *,
    ...
    # MEMORY, NOT ELEGANCE, DRIVES THE SHAPE OF THIS FUNCTION.
    #
    # v31 built the mask on a re-sorted COPY of the panel, then returned
    # `p.loc[keep].sort_values(...).reset_index(drop=True)` - three more
    # copies. At 1.95M rows x 138 float columns pandas consolidates each one
    # into a single ~2 GB block, and with the original frame still live the
    # peak blew past available RAM:
    #     ArrayMemoryError: Unable to allocate 2.00 GiB for (138, 1949070)
    #
    # So: compute the mask WITHOUT copying the frame, filter once, and leave
    # sorting and reset_index to the single call build_panel already makes at
    # the end.
    order = np.lexsort((panel["timestamp"].to_numpy(),
                        panel["symbol"].to_numpy()))
    dv = pd.to_numeric(panel["D_dollar_vol"], errors="coerce").to_numpy()[order]
    syms_sorted = panel["symbol"].to_numpy()[order]

    med_sorted = (pd.Series(dv)
                  .groupby(pd.Series(syms_sorted), sort=False)
                  .rolling(window, min_periods=min_obs).median()
                  .reset_index(level=0, drop=True)
                  .to_numpy())

    med = np.empty(len(panel), dtype="float64")
    med[order] = med_sorted
    del dv, syms_sorted, med_sorted, order

    keep = med >= min_turnover          # NaN >= x is False: too little history
    dropped = int((~keep).sum())
    if verbose:
        surviving = panel.loc[keep, "symbol"].nunique()
    ...
    # Boolean .loc already returns a new frame; an extra .copy() here would
    # double the peak for nothing.
    out = panel.loc[keep]
    out["X_turnover_med"] = med[keep]
    return out
    ...
    *,
    ...
    if not full and out_pq.exists():
        ...
        if not existing.empty:
            ...
            # WHAT GETS LOADED - deliberately wider.
            #
            # v30 bug: the slice was exactly `span` (20) sessions, but the
            # liquidity floor needs a LIQ_WINDOW (60) session trailing median
            # of turnover. On a 20-session slice that median is NaN for every
            # row, every row failed the floor, and the build died with
            # "liquidity floor removed every row" - which read like a floor
            # that was set too high rather than a window that was too short.
            #
            # Any trailing-window computation between here and the merge has
            # the same requirement, so load LIQ_WINDOW extra sessions of
            # context and trim back to `since` before merging.
            load_idx = max(0, cutoff_idx - LIQ_WINDOW)
            load_since = pd.Timestamp(sessions[load_idx]).tz_localize(None)
            if verbose:
                print(f"[panel] incremental: rewriting from {since.date()} "
                      f"({span} sessions), loading from "
                      f"{load_since.date()} so the {LIQ_WINDOW}-session "
                      f"turnover median is computable")
    if full and verbose:
    ...
    for i, s in enumerate(equities, 1):
        try:
            f = symbol_frame(root, s, load_since)
            if not f.empty:
                frames.append(f)
    ...
    # Downcast float64 -> float32 before anything else touches the frame.
    #
    # Every feature here is derived from prices and volumes; float32 carries
    # ~7 significant digits, which is far more than a ratio of two prices
    # means. float64 doubles the memory for no information: at 1.95M rows the
    # difference is ~1 GB per consolidated block, and pandas allocates whole
    # blocks at a time during copies. LABEL columns are left alone - they are
    # small, and one of them is the thing being predicted.
    fcols = [c for c in panel.columns
             if c not in PANEL_LABELS and panel[c].dtype == "float64"]
    if fcols:
        panel[fcols] = panel[fcols].astype("float32")
    if verbose:
        mb = panel.memory_usage(deep=False).sum() / 1e6
        print(f"[panel] stacked {len(panel):,} rows x {panel.shape[1]} cols "
              f"({mb:,.0f} MB, {len(fcols)} cols float32)")
    ...
    if panel.empty:
        raise RuntimeError(
            ...
            f"  If this is an INCREMENTAL build, the usual cause is too few "
            f"sessions loaded for the {LIQ_WINDOW}-session turnover median - "
            f"not a floor set too high.\n"
            f"  Otherwise lower the floor or set it to 0."
        )
    ...
    # Trim the lookback context back off: it was loaded only so the trailing
    # windows above could be computed, and those rows already exist in the
    # stored panel.
    if since is not None and load_since is not None and load_since < since:
        before = len(panel)
        panel = panel.loc[panel["timestamp"] >= since]
        if verbose:
            print(f"  dropped {before - len(panel):,} lookback-context rows "
                  f"before {since.date()} (already in the stored panel)")
...
def label_contract() -> dict:
    """The target, stated once, exactly as compute_labels implements it."""
    suf, tp, sl, atr = next(v for v in LABEL_VARIANTS if v[0] == PRIMARY_VARIANT)
    return {
        "primary_variant": PRIMARY_VARIANT,
        "take_profit": f"{tp} x ATR14" if atr else f"{tp:.1%}",
        "stop_loss": f"{sl} x ATR14" if atr else f"{sl:.1%}",
        "atr_definition": ("ATR14 = SIMPLE 14-day mean of true range, divided by "
                           "close at T. NOT D_atr14, which is Wilder-smoothed."),
        "entry": "close of session T",
        "horizon_sessions": LABEL_HORIZON,
        "path": "sessions T+1..T+H, daily high vs TP level, daily low vs SL level",
        "same_day_tie": "both barriers crossed in one session -> counted as SL "
                        "(order unknowable from daily bars)",
        "label_exit_ret": "+TP if TP first; -SL if SL first (ties = SL); else "
                          "close[T+H]/close[T]-1",
        "label_touch": f"high reaches +{LABEL_TOUCH_PCT:.0%} within H (fixed %, "
                       f"independent of the bracket)",
        "pending": "last H sessions of each symbol are NaN, never negative",
        "variants": [{"suffix": v[0], "tp": v[1], "sl": v[2], "atr_scaled": v[3]}
...
def panel_feature_columns(panel: pd.DataFrame) -> List[str]:
    ...
    banned |= {c for c in panel.columns if RC.is_forbidden_feature(c)}
    out = [c for c in panel.columns if c not in banned]
    RC.assert_no_label_leak(out, "panel_feature_columns")
    return out

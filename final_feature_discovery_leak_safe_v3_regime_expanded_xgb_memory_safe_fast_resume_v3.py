...
# Memory/performance implementation detail only. ArrayManager stores each column
# independently, preventing pandas BlockManager from attempting to consolidate the
# ~1,250 float32 expansion columns into one contiguous ~9 GiB temporary ndarray.
# This does not alter values, formulas, ordering, folds, models, thresholds, or
# any research logic. Pandas 2.2.x supports this manager; if unavailable, the
# script falls back to the normal manager.
try:
    if hasattr(pd.options.mode, "data_manager"):
...
VERSION = "2026-09-25.master.v3.regime_expanded.xgb_memory_safe_fast_resume_v3"
SEED = 1729
...
# Performance-only batching. This does not change which features are created
# or how they are calculated; it limits the size of temporary pandas blocks.
EXPANSION_BATCH_SOURCE_FEATURES = 16


def progress_path(out: Path, key: str, signature: str) -> Path:
    return out / "checkpoints" / f"{safe_name(key)}_{signature[:16]}.jsonl"


def reset_progress(out: Path, key: str, signature: str) -> None:
    p = progress_path(out, key, signature)
    if p.exists():
        p.unlink()


def load_progress(out: Path, key: str, signature: str) -> dict[str, dict]:
    p = progress_path(out, key, signature)
    done: dict[str, dict] = {}
    if not p.exists():
        return done
        ...
        with p.open("r", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    # An interrupted process can leave a partial final line.
                    # Ignore only that malformed record and retain prior work.
                    continue
    except OSError:
        return {}
    return done
...
def append_progress(out: Path, key: str, signature: str, item: str, payload: dict) -> None:
    p = progress_path(out, key, signature)
    p.parent.mkdir(parents=True, exist_ok=True)
    rec = {"item": str(item), "completed_at": time.strftime("%Y-%m-%d %H:%M:%S"), **payload}
...
def performance_timer(label: str):
    start = time.perf_counter()
    class _Timer:
        def stop(self):
            elapsed = time.perf_counter() - start
            log(f"PERF | {label}: {elapsed/60.0:.2f} min")
            return elapsed
    return _Timer()
...
def add_existing_panel_feature_expansions(
    ...
    """Expand original numeric panel features without giant concat/consolidation.

    IMPORTANT: this is an implementation-only optimization. The feature set,
    formulas, ordering, names, and candidate metadata are unchanged.

    We deliberately assign each generated column directly to the DataFrame.
    With pandas ArrayManager enabled above, each column remains an independent
    array, so pandas never attempts the ~9 GiB BlockManager consolidation that
    caused the previous crash. This is an internal storage optimization only;
    feature values, names, order, and all downstream research logic are unchanged.
    """
    x = work_df
    base_id = x["_row_id"].to_numpy(copy=False)
    ...
    candidates: list[dict] = []
    existing_names = set(x.columns)
    symbol_g = x.groupby("symbol", sort=False)
    date_g = x.groupby("_date", sort=False)
    staged_count = 0
    ...
    for c in source_cols:
        ...
        # Use the original panel column for the source calculation. It is
        # unchanged throughout this function and avoids repeatedly looking up
        # a column in an increasingly fragmented working DataFrame.
        s = pd.to_numeric(panel_df[c], errors="coerce")
        lag1 = symbol_g[c].shift(1)
        lag5 = symbol_g[c].shift(5)
        mu20 = (
            symbol_g[c]
        ...
        xrank = date_g[c].rank(pct=True, method="average")
        ...
        for name, series, family, lb, hyp in specs:
            ...
            # Direct assignment is intentional. DO NOT replace this with a
            # giant pd.concat: that is the 9 GiB failure we are eliminating.
            x[name] = pd.to_numeric(series, errors="coerce").astype("float32")
        ...
        if staged_count and staged_count % 100 == 0:
            log(f"Existing-panel feature expansion attached: {staged_count:,} columns")

        del lag1, lag5, mu20, sd20, xrank, specs, s

    log(f"Existing-panel feature expansion complete: {staged_count:,} columns attached")
...
def stage_c_screen(
    ...
    key = f"{target}_stageC_progress"
    if force:
        reset_progress(out, key, signature)
    done = load_progress(out, key, signature)
    rows = [v["result"] for v in done.values() if "result" in v]
    ...
    for i, c in enumerate(candidates["feature"], 1):
        if c in done:
            continue
        if i % 25 == 0 or i == 1:
            log(f"Stage C {target}: {i}/{total} | resume={len(done):,} complete")
        try:
            r = standalone_oos(df, c, target, folds)
            result = {"feature": c, **r}
        except Exception as e:
            result = {"feature": c, "error": repr(e), "folds": 0}
        append_progress(out, key, signature, c, {"result": result})
...
def stage_d_incremental(
    ...
    """Incremental OOS vs baseline with exact same calculations, plus resume.

    Performance-only changes:
      * baseline model is fitted once per fold and reused;
      * fold masks and cleaned baseline/test frames are reused;
      * candidates checkpoint after completion, so interruption resumes at the
        last completed candidate rather than restarting the stage.
    """
    candidates = stage_c["feature"].tolist()
    key = f"{target}_stageD_progress"
    if force:
        reset_progress(out, key, signature)
    done = load_progress(out, key, signature)
    rows = [v["result"] for v in done.values() if "result" in v]
    ...
    # Build the invariant baseline/test state once. Values are unchanged from
    # fit_predict(): only +/-inf -> NaN is applied, exactly as before.
    fold_states = []
    for f in folds:
        ...
        xtr0 = trbase[baseline].replace([np.inf, -np.inf], np.nan)
        xte0 = tebase[baseline].replace([np.inf, -np.inf], np.nan)
        model0 = make_baseline_model()
        model0.fit(xtr0, ytr.to_numpy())
        p0 = model0.predict_proba(xte0)[:, 1]
        auc0, ll0 = metric_pair(yte.to_numpy(), p0)
        ...
        del xtr0, xte0, model0, p0

    for i, c in enumerate(candidates, 1):
        if c in done:
            continue
        if i % 10 == 0 or i == 1:
            log(f"Stage D {target}: candidate {i}/{len(candidates)} | resume={len(done):,} complete")
        fold_rows = []
        for st in fold_states:
            try:
                cols1 = baseline + [c]
                xtr1 = st["trbase"][cols1].replace([np.inf, -np.inf], np.nan)
                xte1 = st["tebase"][cols1].replace([np.inf, -np.inf], np.nan)
                p1 = fit_predict(make_baseline_model(), xtr1, st["ytr"], xte1)
                auc1, ll1 = metric_pair(st["yte"].to_numpy(), p1)
                hit1 = top1_hit_rate(st["tebase"], st["yte"].to_numpy(), p1)
                fold_rows.append({
                    "fold": st["fold"],
                    "auc_base": st["auc0"],
                    "auc_plus": auc1,
                    "delta_auc": auc1 - st["auc0"],
                    "logloss_base": st["ll0"],
                    "logloss_plus": ll1,
                    "delta_logloss": st["ll0"] - ll1,
                    "top1_base": st["hit0"],
                    "top1_plus": hit1,
                    "delta_top1": hit1 - st["hit0"],
                    "n_test": len(st["tebase"]),
                })
                del xtr1, xte1, p1
            except Exception as e:
                fold_rows.append({"fold": st["fold"], "error": repr(e)})

        fr = pd.DataFrame(fold_rows)
        good = fr.loc[fr["delta_auc"].notna()].copy() if not fr.empty else pd.DataFrame()
        if good.empty:
            result = {"feature": c, "folds": 0}
        else:
            result = {
                "feature": c,
                "folds": int(len(good)),
...
def stage_regime_specialists(
    ...
    """Regime-specific discovery with exact same models/statistics plus resume."""
    candidates = stage_c["feature"].tolist()
    fold_cache = _cached_fold_indices(df, folds)
    key = f"{target}_stageG_progress"
    if force:
        reset_progress(out, key, signature)
    done = load_progress(out, key, signature)
    rows = [v["result"] for v in done.values() if "result" in v]

    # Candidate order is preserved. Each item is one regime/candidate result.
    for regime in REGIME_NAMES:
        ...
        for f in folds:
            ...
            if ytr.nunique() < 2 or yte.nunique() < 2:
                continue
            xtr0 = trdf[baseline].replace([np.inf, -np.inf], np.nan)
            xte0 = tedf[baseline].replace([np.inf, -np.inf], np.nan)
            model0 = make_baseline_model()
            model0.fit(xtr0, ytr.to_numpy())
            p0 = model0.predict_proba(xte0)[:, 1]
            auc0, ll0 = metric_pair(yte.to_numpy(), p0)
            regime_states.append({
                "fold": f.fold, "trdf": trdf, "tedf": tedf,
                "ytr": ytr, "yte": yte, "auc0": auc0, "ll0": ll0,
            })
            del xtr0, xte0, model0, p0
        ...
        for i, c in enumerate(candidates, 1):
            ...
            if item_key in done:
                continue
            if i % 10 == 0 or i == 1:
                log(f"Regime {target}/{regime}: candidate {i}/{len(candidates)} | resume={len(done):,} complete")
            fold_rows = []
            for st in regime_states:
                try:
                    cols1 = baseline + [c]
                    xtr1 = st["trdf"][cols1].replace([np.inf, -np.inf], np.nan)
                    xte1 = st["tedf"][cols1].replace([np.inf, -np.inf], np.nan)
                    p1 = fit_predict(make_baseline_model(), xtr1, st["ytr"], xte1)
                    auc1, ll1 = metric_pair(st["yte"].to_numpy(), p1)
                    fold_rows.append({
                        "fold": st["fold"], "regime": regime,
                        "auc_base": st["auc0"], "auc_plus": auc1,
                        "delta_auc": auc1 - st["auc0"],
                        "logloss_base": st["ll0"], "logloss_plus": ll1,
                        "delta_logloss": st["ll0"] - ll1,
                        "n_train": len(st["trdf"]), "n_test": len(st["tedf"]),
                    })
                    del xtr1, xte1, p1
                except Exception as e:
                    fold_rows.append({"fold": st["fold"], "regime": regime, "error": repr(e)})
            fr = pd.DataFrame(fold_rows)
            good = fr.loc[fr["delta_auc"].notna()].copy() if not fr.empty else pd.DataFrame()
            if good.empty:
                result = {"feature": c, "regime": regime, "folds": 0}
            else:
                result = {
                    "feature": c, "regime": regime,
                    "folds": int(len(good)),
                    "mean_delta_auc": float(good["delta_auc"].mean()),
                    "median_delta_auc": float(good["delta_auc"].median()),
                    "min_delta_auc": float(good["delta_auc"].min()),
            ...
            append_progress(out, key, signature, item_key, {"result": result})
            done[item_key] = {"result": result}
            rows.append(result)
...
def main():
    ...
    manifest = {
        ...
        "performance_optimizations": {
            "expansion_attachment": "direct_column_assignment_with_array_manager_no_global_concat",
            "candidate_level_resume": True,
            "baseline_fit_reuse": True,
        },
    }
    ...
    if stage_done(out, key, sig) and not args.force:
        ...
        t_feature_perf = performance_timer("Feature construction")
    ...
    for target in targets:
        ...
        if stage_done(out, key_a, sig_a) and not args.force:
            ...
            t_perf = performance_timer(f"{target} Stage A")
        ...
        if stage_done(out, key_b, sig_b) and not args.force:
            ...
            t_perf = performance_timer(f"{target} Stage B")
        ...
        if stage_done(out, key_c, sig_c) and not args.force:
            ...
            t_perf = performance_timer(f"{target} Stage C")
            stage_c = stage_c_screen(work, stage_b, target, folds, out, sig_c, force=args.force)
            t_perf.stop()
        ...
        if stage_done(out, key_g, sig_g) and not args.force:
            ...
            t_perf = performance_timer(f"{target} Stage G")
            regime_selected = stage_regime_specialists(
                work, baseline, stage_c, target, folds, out, sig_g, force=args.force
            )
            t_perf.stop()
        ...
        if stage_done(out, key_d, sig_d) and not args.force:
            ...
            t_perf = performance_timer(f"{target} Stage D")
            stage_d = stage_d_incremental(
                work, baseline, stage_c, target, folds, out, sig_d, force=args.force
            )
            t_perf.stop()

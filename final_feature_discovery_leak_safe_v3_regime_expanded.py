...
VERSION = "2026-09-25.master.v3.regime_expanded"
SEED = 1729
...
def add_existing_panel_feature_expansions(
    ...
    """
    FEATURE-ADDITION-ONLY EXTENSION.

    Generate a bounded, deterministic library of causal transformations for
    numeric features that already existed in panel.parquet BEFORE this script
    added any FD_* / REG_* columns.

    This function deliberately does not alter:
      - targets/labels
      - folds, purge, embargo
      - Stage A-F logic
      - baseline selection
      - thresholds
      - existing 44-feature library

    Source eligibility uses the SAME static leak-name gate already used by the
    script. Each transformation uses only source values available at date t,
    so the resulting feature is usable at t+1.

    Added families per eligible continuous panel feature:
      1) lag-1 value
      2) 5-session first difference
      3) 20-session rolling z-score
      4) same-date cross-sectional percentile rank

    The source list is captured from panel_df, not work_df, so this function
    cannot recursively transform its own newly-created features.
    """
    x = work_df.copy()
    base_id = x["_row_id"].to_numpy(copy=True)
    candidates: list[dict] = []
    ...
    # Only columns that were already present in the original panel.
    source_cols = [
    ...
    # Also test the raw existing panel feature itself when it is not already
    # part of the approved baseline. This does not copy the column; it simply
    # adds the already-present panel feature to the candidate inventory.
    ...
    def add_feature(name: str, series, source: str, family: str,
                    lookback: int, hypothesis: str):
        if name in x.columns:
            return
    ...
    for i, c in enumerate(source_cols, 1):
        ...
        # 1) One-session lag: strictly historical relative to t.
        lag1 = s.groupby(x["symbol"], sort=False).shift(1)
        add_feature(
            f"FD_PX_LAG1__{safe_name(c)}",
            lag1,
            c,
            "existing_feature_lag",
            1,
            f"one-session lag of existing panel feature {c}",
        )

        # 2) Five-session first difference.
        lag5 = s.groupby(x["symbol"], sort=False).shift(5)
        add_feature(
            f"FD_PX_D5__{safe_name(c)}",
            s - lag5,
            c,
            "existing_feature_change",
            5,
            f"five-session change of existing panel feature {c}",
        )

        # 3) Causal 20-session rolling z-score. Current t is included,
        # consistent with the existing feature-zscore convention.
        mu20 = (
            s.groupby(x["symbol"], sort=False)
        ...
        add_feature(
            ...
            c,
            "existing_feature_normalization",
            20,
            f"20-session causal z-score of existing panel feature {c}",
        )

        # 4) Same-date cross-sectional percentile rank. This is computed from
        # date-t values only and therefore is available after the t close.
        xrank = x.groupby("_date")[c].rank(pct=True, method="average")
        add_feature(
            f"FD_PX_XRANK__{safe_name(c)}",
            xrank,
            c,
            "existing_feature_cross_section",
            1,
            f"same-date cross-sectional rank of existing panel feature {c}",
        )

        if i % 25 == 0:
            log(f"Existing-panel feature expansion: {i}/{len(source_cols)} sources")

    if not np.array_equal(base_id, x["_row_id"].to_numpy()):
        raise RuntimeError(
            "FATAL: row-id misalignment after existing-panel feature expansion."
        )
...
def fit_predict(
    ...
    # HGB cannot consume NaN? Current sklearn HGB can, but explicit median
    # imputation makes behavior deterministic and keeps all models comparable.
    imp = SimpleImputer(strategy="median")
    a = imp.fit_transform(xtr)
    b = imp.transform(xte)

    model.fit(a, ytr.astype(int).to_numpy())
...
def stage_d_incremental(
    ...
    for i, c in enumerate(candidates, 1):
        if i % 10 == 0:
            log(f"Stage D {target}: {i}/{len(candidates)}")

        try:
            r = incremental_oos(df, baseline, c, target, folds)
            rows.append({"feature": c, **r})
        except Exception as e:
            rows.append({"feature": c, "folds": 0, "error": repr(e)})

    r = pd.DataFrame(rows)
    if r.empty:
        raise RuntimeError(f"Stage D produced no results for {target}.")

    # Require stability, not just one lucky fold.
    r["pass_incremental"] = (
        (r["folds"] >= N_FOLDS)
...
def empirical_max_null(
    ...
    """
    Conservative empirical null.

    For each permutation, shuffle ONLY the candidate columns across rows
    WITHIN EACH DATE, leaving the real target and baseline features intact.
    This destroys candidate/target association while preserving the real
    baseline relationship. We then test all candidates against the same
    baseline model and record the maximum mean delta-AUC.

    This is intentionally much more expensive than a simple feature shuffle,
    but it actually tests the multiple-testing/discovery process.
    """
    rng = np.random.default_rng(SEED + 1000)
    max_scores = []

    # To keep the 10h+ run bounded, the empirical null uses the final shortlist.
    # The real discovery stage has already reduced hundreds of candidates.
    for pno in range(permutations):
        log(f"Null {target}: permutation {pno + 1}/{permutations}")

        # Correct incremental-null construction:
        # keep the real target and baseline intact, and destroy only the
        # candidate -> target relationship. This asks whether the candidate
        # can appear to add incremental OOS edge by chance while the real
        # baseline relationship remains present.
        shuffled = df.copy()
        for c in candidate_names:
            shuffled[c] = (
                shuffled.groupby("_date", sort=False)[c]
                .transform(
                    lambda s: s.sample(
                        frac=1.0,
        ...
        for c in candidate_names:
            try:
                r = incremental_oos(shuffled, baseline, c, target, folds)
                score = r.get("mean_delta_auc", np.nan)
                if pd.notna(score):
        ...
        atomic_write_json(
            ...
            out / f"null_{safe_name(target)}.json",
        )
...
def add_causal_regimes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if atr is not None:
        ...
        x["REG_STOCK_VOL_HIGH"] = (
            atr.notna() & atr_q.notna() & (atr > atr_q)
        ).astype("int8")
    else:
        x["REG_STOCK_VOL_HIGH"] = np.int8(0)
...
def stage_regime_specialists(
    ...
    """
    Evaluate Stage-C candidates independently inside each stock regime.
    This deliberately happens before universal Stage-D filtering so a feature
    that only works in one regime is not discarded for being weak overall.
    """
    rows = []
    candidates = stage_c["feature"].tolist()
    ...
    for regime in REGIME_NAMES:
        ...
        for i, c in enumerate(candidates, 1):
            if i % 25 == 0:
                log(f"Regime discovery {target}/{regime}: {i}/{len(candidates)}")
            try:
                r = regime_incremental_oos(df, baseline, c, target, folds, regime)
                rows.append({"feature": c, "regime": regime, **r})
            except Exception as e:
                rows.append({
                    "feature": c,
                    "regime": regime,

...
VERSION = "2026-09-25.master.v3.regime_expanded.xgb_memory_safe_fast_resume_v2"
SEED = 1729
...
def add_existing_panel_feature_expansions(
    ...
    A large ``pd.concat`` forces pandas to consolidate roughly
    1.95M x 1,250 float32 values into a temporary ~9 GiB ndarray. Direct
    assignment avoids that temporary allocation. The resulting DataFrame is
    intentionally fragmented; downstream code selects only the small feature
    blocks it needs, so global consolidation is never requested.
    """
    x = work_df
    base_id = x["_row_id"].to_numpy(copy=False)
...
def main():
    ...
    manifest = {
        ...
        "performance_optimizations": {
            "expansion_attachment": "direct_column_assignment_no_global_concat",

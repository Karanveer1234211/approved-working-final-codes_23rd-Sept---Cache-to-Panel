...
def add_existing_panel_feature_expansions(
    ...
    """
    # work_df is already the working copy owned by the pipeline. Avoid another
    # ~GB-scale DataFrame copy here; adding columns in-place does not alter the
    # frozen panel.parquet.
    x = work_df
    base_id = x["_row_id"].to_numpy(copy=True)
    candidates: list[dict] = []
...
def _shuffle_within_date(values: np.ndarray, date_codes: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    """
    Memory-bounded exact within-date permutation of one candidate column.
    Only one 1-D float array is copied; the multi-GB panel is never copied.
    """
    out = np.asarray(values).copy()
    # Date codes are already stable for the full panel. Sorting once outside
    # the permutation loop would be ideal; this helper is called with cached
    # contiguous groups by the null engine below.
